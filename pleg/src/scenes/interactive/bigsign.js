...
export class BigSign extends Phaser.GameObjects.Zone {
  constructor(
    ...
    // SMALL VERSION
    this.sm_signText = scene.add
      .bitmapText(sm_signX, sm_signY, "pixelop", text, 16, 1)
      .setOrigin(0, 1)
      .setDepth(101)
      .setVisible(false);
    this.sm_signRect = scene.add
      .rectangle(
        sm_signX - 10,
        sm_signY,
        this.sm_signText.width + 20,
        this.sm_signText.height,
        0xffffff
      )
    ...
    if (scene.scene.key === "ResearchScene") {
      this.purple_tiles.push(
        scene.add
          .sprite(x - 30, y - 10, "anims_ui", "purple.000")
          .setOrigin(0, 1)
      );
    } else {
      this.purple_tiles.push(
      ...
      if (height > 32) {
        this.purple_tiles.push(
          scene.add
            .sprite(x - 8, y - 32, "anims_ui", "purple.000")
            .setOrigin(0, 1)
        );
        this.purple_tiles.push(
          scene.add
            .sprite(x - 8 + 32, y, "anims_ui", "purple.000")
            .setOrigin(0, 1)
        );
        this.purple_tiles.push(
          scene.add
            .sprite(x - 8 + 32, y - 32, "anims_ui", "purple.000")
            .setOrigin(0, 1)
        );
      }
    }
    this.purple_tiles.forEach((purple_tile) =>
      purple_tile.setDepth(2).play("purple-tile-anim", true)
    );
  ...
  showSignText(player) {
    ...
    if (Math.ceil(player.y + 20) <= this.y) {
      this.purple_tiles.forEach((purple_tile) =>
        purple_tile.anims.stop().setTint(0xffff00)
      );
      this.activated = true;
  ...
  hideSignText(player) {
    if (this.activated) {
      // So that the following code does not execute at every frame, but only once to hide the text
