...
export class Sign extends Phaser.GameObjects.Zone {
    ...
    if (!this.activated) {
      ...
      if (distance < this.clickRadius) {
        if (this.scene.scene.key === "UniversityScene")
          this.scene.signs.forEach((other_sign) => other_sign.hideSignText()); // So there is no clutter in the classroom
        this.showSignText();
  ...
  playerMovement(moveleft, moveright, moveup, movedown) {
    ...
    if (this.showByClick) {
      ...
      // Otherwise, the player activated the sign via collision
      if (moveleft || moveright) {
        this.hideSignText();
      } else if (this.direction === "up" && movedown) {
        this.hideSignText();
      } else if (
        (this.direction === "down" || this.direction === "center") &&
        !movedown
      ) {
        this.hideSignText();
      }
